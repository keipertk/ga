...
#ifdef USE_DEVICE_MEM
int ARMCI_Malloc(void **ptr_arr, armci_size_t bytes, int onDev)
{
    return PARMCI_Malloc(ptr_arr, bytes, onDev);
}
#else
...
#ifdef USE_DEVICE_MEM
int ARMCI_MallocG(void **ptr_gpu_arr, armci_size_t bytes, int dev_id)
{
    return PARMCI_MallocG(ptr_gpu_arr, bytes, dev_id);
}
